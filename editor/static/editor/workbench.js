(() => {
  ...
  if (copyProgress && root.dataset.progressUrl) {
    const pollProgress = async () => {
      try {
        const response = await fetch(root.dataset.progressUrl, { cache: "no-store", headers: { Accept: "application/json" } });
        ...
        if (data.status === "active" || data.status === "failed") {
          ...
          destination.searchParams.set("_vibe_copy_finished", String(Date.now()));
          window.location.replace(destination.href);
          return;
        }
        window.setTimeout(pollProgress, 650);
