...
- wyszukiwane podpowiedzi URL z menu, podmenu, treści i stopki strony głównej;
...
Formularz nowej rozmowy pobiera linki z `homepage_url`, usuwa adresy zewnętrzne i duplikaty oraz rozpoznaje hierarchię typowych menu. Jeśli `homepage_url` nie występuje w TOML, używany jest HTTPS i pierwsza domena bez `www` z `allowed_hosts`. Wynik jest cache'owany przez 5 minut; niedostępność strony głównej nie blokuje ręcznego wpisania URL-u.
